      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--gs-text-secondary)]">
            Repository language counts, grouped into the top languages and
            Other.
          </p>
        </div>
        <div className="text-xs uppercase tracking-[0.18em] text-[var(--gs-text-secondary)]">
          {total} language-tagged repos
        </div>
      </div>
        ...
        >
          {data.map((entry, index) => {
            ...
            return (
              <div
                key={entry.name}
                className="flex items-center justify-between rounded-lg border border-[var(--gs-border)] bg-[var(--gs-surface-alt)] px-4 py-3"
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  />
                  <span className="truncate text-sm font-medium text-[var(--gs-text)]">
                    {entry.name}
                  </span>
                </div>
                <span className="text-sm text-[var(--gs-text-secondary)]">
                  {percent}%
                </span>
              </div>
            );
