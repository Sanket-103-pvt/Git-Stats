...
export default function PlayerCardModal({
  ...
  const activeDays = Object.values(activityMap || {}).filter(
    (v) => v > 0,
  ).length;
  ...
  const topRepoReach = Math.max(
    0,
    ...repos.map((r) => r.stargazers_count || 0),
  );
  ...
  const pac = Math.min(
    99,
    Math.max(50, 50 + Math.round(Math.min(49, activeDays * 0.8))),
  );

  // Shooting (SHO): Based on total stars (log scale)
  const sho = Math.min(
    99,
    Math.max(50, 50 + Math.round(Math.min(49, 12 * Math.log2(totalStars + 1)))),
  );
  ...
  const pas = Math.min(
    ...
    Math.max(
      50,
      50 +
        Math.round(Math.min(49, 10 * Math.log2((profile.followers || 0) + 1))),
    ),
  );
  ...
  const dri = Math.min(
    99,
    Math.max(50, 50 + Math.round(Math.min(49, uniqueLangsCount * 7))),
  );

  // Defending (DEF): Based on forks (log scale)
  const def = Math.min(
    99,
    Math.max(50, 50 + Math.round(Math.min(49, 10 * Math.log2(totalForks + 1)))),
  );

  // Physical (PHY): Based on public repos and account age
  const years =
    (new Date() - new Date(profile.created_at || Date.now())) /
  ...
  const phy = Math.min(
    ...
    Math.max(
      ...
      50 +
        Math.round(
          Math.min(49, (profile.public_repos || 0) * 0.7 + years * 4.0),
        ),
    ),
  );
    ...
    const patterns = [
      {
        ...
        count: nightCount,
        icon: Moon,
        desc: "commits between 10 PM and 5 AM",
      },
      {
        name: "Early Bird",
        count: earlyCount,
        icon: Sun,
        desc: "commits between 5 AM and 9 AM",
      },
      {
        name: "Day Grinder",
        count: dayCount,
        icon: Activity,
        desc: "commits between 9 AM and 6 PM",
      },
      {
        name: "Evening Coder",
        count: eveningCount,
        icon: Clock,
        desc: "commits between 6 PM and 10 PM",
      },
    ];
  ...
  if (eventTimestamps && eventTimestamps.length >= 10) {
    ...
    const varHr =
      hours.reduce((a, b) => a + Math.pow(b - meanHr, 2), 0) / hours.length;
  ...
  if (uniqueLangsCount > 3) {
    playstyles.push({
      name: "Polyglot",
      desc: "Working across multiple stacks",
      icon: Globe,
    });
  }
  if (totalForks > 5) {
    playstyles.push({
      name: "Connector",
      desc: "Highly collaborative developer",
      icon: GitPullRequest,
    });
  }
  if (totalContributions > 200) {
    playstyles.push({
      name: "Rapid Fire",
      desc: "Extremely high commit velocity",
      icon: Flame,
    });
  }
  if (years > 3) {
    playstyles.push({
      name: "Maintainer",
      desc: "Long-term repository steward",
      icon: Shield,
    });
  }
  ...
  if (playstyles.length < 3) {
    playstyles.push({
      name: "Tactician",
      desc: "Clean architecture design",
      icon: Sparkles,
    });
  }
  ...
  const issuesCount = Math.round(
    activeDays * 0.15 + (profile.public_repos || 0) * 0.3,
  );
  ...
  const rawMetrics = [
    {
      label: "Commits",
      raw: `${totalContributions} commits`,
      score: Math.min(
        99,
        Math.max(50, 50 + Math.round(Math.min(49, totalContributions * 0.1))),
      ),
    },
    { label: "Stars earned", raw: `${totalStars} stars`, score: sho },
    {
      label: "Top repo reach",
      raw: `${topRepoReach} stars`,
      score: Math.min(
        99,
        Math.max(
          50,
          50 + Math.round(Math.min(49, 15 * Math.log2(topRepoReach + 1))),
        ),
      ),
    },
    {
      ...
      score: Math.min(
        99,
        Math.max(50, 50 + Math.round(Math.min(49, prsCount * 2.5))),
      ),
    },
    {
      label: "Followers",
      raw: `${profile.followers || 0} followers`,
      score: pas,
    },
    {
      label: "Languages",
      raw: `${uniqueLangsCount} langs`,
      score: Math.min(
        99,
        Math.max(50, 50 + Math.round(Math.min(49, uniqueLangsCount * 7.5))),
      ),
    },
    {
      label: "Issues",
      raw: `${issuesCount} issues`,
      score: Math.min(
        99,
        Math.max(50, 50 + Math.round(Math.min(49, issuesCount * 2.0))),
      ),
    },
    {
      label: "Code reviews",
      raw: `${codeReviews} reviews`,
      score: Math.min(
        99,
        Math.max(50, 50 + Math.round(Math.min(49, codeReviews * 3.0))),
      ),
    },
    {
      label: "Contributions",
      raw: `${totalContributions} total`,
      score: Math.min(
        99,
        Math.max(50, 50 + Math.round(Math.min(49, totalContributions * 0.08))),
      ),
    },
  ];
          ...
          <div className="fixed inset-0 z-[9999] bg-slate-950 overflow-y-auto select-none font-sans text-slate-100">
            ...
            <div className="sticky top-0 bg-slate-950/80 backdrop-blur-md border-b border-slate-800 z-50 px-6 py-4 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <span className="text-xl font-bold tracking-wider text-[var(--gs-accent)]">
                  SCOUTING REPORT
                </span>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex flex-col items-center justify-center p-3 rounded-xl border border-white/10 bg-slate-900 shadow-md min-w-[70px] text-center">
                    ...
                    <div
                      className={`text-[9px] font-black uppercase tracking-wider mt-1 ${theme.text}`}
                    >
                      {theme.name}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2.5 mt-1.5 text-xs">
                      <span
                        className={`px-2 py-0.5 rounded font-black text-[10px] bg-slate-800 border border-white/10 ${theme.text}`}
                      >
                        {position}
                      </span>
                ...
                <div
                  className={`md:max-w-md ${theme.btnBg} border ${theme.btnBorder} rounded-xl p-4 shadow-lg animate-fade-in-up duration-500`}
                >
                  <div className="flex items-center gap-1.5 text-[9px] font-black tracking-widest text-slate-400 uppercase">
                    <Sparkles
                      className={`h-3 w-3 ${theme.text} animate-pulse`}
                    />
                    <span>CODER PERSONALITY INSIGHT</span>
                  </div>
                  <h3
                    className={`text-sm font-black tracking-wide uppercase mt-1 ${theme.text}`}
                  >
                    {insight.title}
                  </h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_320px] gap-8 items-start">
                  ...
                  <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 space-y-4 shadow-xl">
                    ...
                    <div className="space-y-3.5 text-sm">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex text-amber-400 text-xs tracking-widest">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              className={`h-3 w-3 ${i < skillMoves ? "fill-current" : "opacity-20"}`}
                            />
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-black tracking-widest uppercase bg-slate-800 border border-slate-700 ${theme.text}`}
                        >
                          {styleTag}
                        </span>
                      </div>
                      ...
                      {playstyles.map((ps, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex items-start gap-3">
                            <div
                              className={`p-2 rounded-lg bg-slate-800 border border-slate-700/60 ${theme.text}`}
                            >
                ...
                <div className="flex flex-col items-center justify-center space-y-6">
                  ...
                  <div
                    ...
                    style={{
                      clipPath:
                        "polygon(50% 0%, 100% 12%, 100% 80%, 50% 100%, 0% 80%, 0% 12%)",
                    }}
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start mt-6">
                        ...
                        <div className="flex flex-col items-start pl-3 pt-3 space-y-0.5">
                          ...
                          <div
                            className={`text-xs font-black tracking-widest uppercase opacity-90 ${theme.text}`}
                          >
                            {position}
                          </div>
                        ...
                        <div className="relative pr-2 pt-2">
                          <div
                            className={`h-32 w-32 rounded-full border-2 ${theme.photoBorder} overflow-hidden shadow-[inset_0_2px_8px_rgba(0,0,0,0.6)] ring-4 ring-black/35 bg-transparent`}
                          >
                            <img
                              src={avatarUrl}
                              alt={profile.login}
                      ...
                      <div className="grid grid-cols-3 gap-y-3 gap-x-2 px-4 mb-6 text-center">
                        <div className="flex flex-col items-center">
                          <span
                            className={`text-base font-black ${getStatColor(pac)}`}
                          >
                            {pac}
                          </span>
                        ...
                        <div className="flex flex-col items-center">
                          <span
                            className={`text-base font-black ${getStatColor(dri)}`}
                          >
                            {dri}
                          </span>
                          <span className="text-[9px] font-bold text-white/50 tracking-wider">
                        ...
                        <div className="flex flex-col items-center">
                          <span
                            className={`text-base font-black ${getStatColor(sho)}`}
                          >
                            {sho}
                          </span>
                          <span className="text-[9px] font-bold text-white/50 tracking-wider">
                        ...
                        <div className="flex flex-col items-center">
                          <span
                            className={`text-base font-black ${getStatColor(def)}`}
                          >
                            {def}
                          </span>
                          <span className="text-[9px] font-bold text-white/50 tracking-wider">
                        ...
                        <div className="flex flex-col items-center">
                          <span
                            className={`text-base font-black ${getStatColor(pas)}`}
                          >
                            {pas}
                          </span>
                          <span className="text-[9px] font-bold text-white/50 tracking-wider">
                        ...
                        <div className="flex flex-col items-center">
                          <span
                            className={`text-base font-black ${getStatColor(phy)}`}
                          >
                            {phy}
                          </span>
                          <span className="text-[9px] font-bold text-white/50 tracking-wider">
                ...
                <div className="space-y-6">
                        ...
                        <div key={idx} className="space-y-1">
                          <div className="flex justify-between items-end text-xs">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span
                                className={`font-mono font-black text-sm leading-none ${getStatColor(metric.score)}`}
                              >
                                {metric.score}
                              </span>
                            </div>
                  ...
                  <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 space-y-4 shadow-xl">
                    ...
                    <div className="relative pt-4">
                      <svg
                        viewBox="0 0 100 30"
                        className="w-full h-16 overflow-visible text-slate-700 fill-slate-800/20"
                      >
                        {/* Bell curve baseline */}
                        <path
                    ...
                    <div className="grid grid-cols-2 gap-3 text-center border-t border-slate-800/80 pt-4">
                      <div className="bg-slate-950/40 p-2.5 rounded-xl border border-slate-800/50">
                        <div className="text-slate-500 text-[9px] font-bold tracking-wider uppercase">
                          Global percentile
                        </div>
                        <div className="text-base font-black text-emerald-400 mt-0.5">
                          TOP {Math.max(1, 100 - ovr)}%
                        </div>
                      </div>
                      <div className="bg-slate-950/40 p-2.5 rounded-xl border border-slate-800/50">
                        <div className="text-slate-500 text-[9px] font-bold tracking-wider uppercase">
                        ...
                        <div className="text-base font-black text-cyan-400 mt-0.5">
                          TOP {Math.max(1, Math.round((100 - ovr) * 1.4))}%
                        </div>
                      </div>
                    </div>
                  </div>
